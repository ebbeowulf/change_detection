...
NEW_POSE_FILE=$BASE_DIR/camera_gps.txt
cmd="python ${PYTHON_HOME}/extract_camera_gps.py $BASE_DIR/gps_log.csv $NEW_POSE_FILE"
echo $cmd
eval $cmd
...
if [[ ! -f $SPARSE_GEO/0/images.bin ]];then
    ...
    cmd="colmap model_aligner --input_path $SPARSE/0 --output_path $SPARSE_GEO/0 --alignment_max_error 1 --ref_is_gps 1 --ref_images_path $NEW_POSE_FILE"
    echo $cmd
    eval $cmd
