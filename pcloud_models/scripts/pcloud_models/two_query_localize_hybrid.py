...
import open3d as o3d
import pickle
from map_utils_hybrid import get_distinct_clusters, object_pcloud
...
def combine_matches(test_file_dict, detection_threshold=0.5, min_cluster_points=10, model_strategy="hybrid"):
    """
    Combine outputs from CLIPSeg and OmDet models using the specified strategy.
    Strategies:
    - 'hybrid': Combines CLIPSeg and OmDet clusters with IOU-based matching
    - 'clipseg-only': Uses only CLIPSeg clusters
    - 'omdet-only': Uses only OmDet clusters
    - 'ensemble': Combines clusters with weighted probabilities
    """
    try:
        # Get CLIPSeg clusters
        save_clipseg = test_file_dict['cluster_main_base']+".clipseg.%0.2f.cl.pkl"%(detection_threshold)
        objects_clipseg = get_clusters(save_clipseg, test_file_dict['raw_main'], detection_threshold, min_cluster_points)
        
        # Get OmDet clusters
        save_omdet = test_file_dict['cluster_main_base']+".omdet.%0.2f.cl.pkl"%(detection_threshold)
        objects_omdet = get_clusters(save_omdet, test_file_dict['raw_main'], detection_threshold, min_cluster_points)
        
        if model_strategy == "clipseg-only":
            objects_main = objects_clipseg
            objects_llm = []
        elif model_strategy == "omdet-only":
            objects_main = objects_omdet
            objects_llm = []
        else:  # hybrid or ensemble
            # For hybrid strategy, treat CLIPSeg as main and OmDet as secondary
            objects_main = objects_clipseg
            objects_llm = objects_omdet
    ...
    for idx0 in range(len(objects_main)):
        ...
        if model_strategy == "hybrid" or model_strategy == "ensemble":
            # For hybrid strategy, include both model stats: 
            # [idx, clipseg_max, clipseg_mean, omdet_max, omdet_mean, scene_prob_raw, scene_prob_norm]
            cl_stats = [idx0, 
                       objects_main[idx0].prob_stats['max'], 
                       objects_main[idx0].prob_stats['mean'], 
                       -1, -1, 
                       test_file_dict['scene_prob_raw'], 
                       test_file_dict['scene_prob_norm']]
            
            # Match with secondary model clusters
            for idx1 in range(len(objects_llm)):
                IOU = calculate_iou(objects_main[idx0].box[0], objects_main[idx0].box[1], 
                                   objects_llm[idx1].box[0], objects_llm[idx1].box[1])
                if IOU > 0.5:
                    # Update with OmDet stats
                    cl_stats[3] = max(cl_stats[3], objects_llm[idx1].prob_stats['max'])
                    cl_stats[4] = max(cl_stats[4], objects_llm[idx1].prob_stats['mean'])
        else:
            # For single model strategies, use simpler stats:
            # [idx, model_max, model_mean, -1, -1, scene_prob_raw, scene_prob_norm]
            cl_stats = [idx0, 
                       objects_main[idx0].prob_stats['max'], 
                       objects_main[idx0].prob_stats['mean'], 
                       -1, -1, 
        ...
        for idxA, annot in enumerate(test_file_dict['annotation']):
            IOU_a = calculate_iou(objects_main[idx0].box[0], objects_main[idx0].box[1], 
                                 np.array(annot[:3]), np.array(annot[3:]))
    ...
    return objects_main, positive_clusters, negative_clusters

def estimate_likelihood(cluster_stats, category):
    """
    Estimate likelihood for detection based on cluster stats and category.
    Categories:
    - 'hybrid-mean': Hybrid model using mean probabilities
    - 'clipseg-mean': CLIPSeg model using mean probabilities
    - 'omdet-mean': OmDet model using mean probabilities
    - 'ensemble': Weighted ensemble of both models
    """
    if category == 'clipseg-mean':
        return cluster_stats[2]  # CLIPSeg mean
    elif category == 'omdet-mean':
        return max(cluster_stats[4], 0)  # OmDet mean, default to 0 if not present
    elif category == 'ensemble':
        # Weighted ensemble - 60% CLIPSeg, 40% OmDet
        clipseg_weight = 0.6
        omdet_weight = 0.4
        clipseg_val = cluster_stats[2]
        omdet_val = max(cluster_stats[4], 0)
        return clipseg_weight * clipseg_val + omdet_weight * omdet_val
    else:  # hybrid-mean or default
        # Return the better of the two models, biased toward CLIPSeg
        clipseg_val = cluster_stats[2]
        omdet_val = max(cluster_stats[4], 0) if cluster_stats[4] > 0 else 0
        
        # If we have values from both models, return the better one
        if clipseg_val > 0 and omdet_val > 0:
            return max(clipseg_val, omdet_val)
        # Otherwise, return whichever value is positive
        return max(clipseg_val, omdet_val)
