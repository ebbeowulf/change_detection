...
class clip_seg_depth(image_segmentation):
    ...
    def process_file(self, fName, threshold=0.2, save_fileName=None):
        ...
        zI=np.zeros((image.shape[0],image.shape[1],3),dtype=int)
        zI[:,:,0]=(image*255.0/3000.0).astype(int)
        # Get the clip probabilities
        outputs = self.process_image(zI)
        # pdb.set_trace()
        # P2=torch.sigmoid(outputs.logits.unsqueeze(0)).to('cpu').numpy()

        self.set_data(outputs,zI.shape,threshold)
    ...
    def set_data(self, outputs, image_size, threshold=0.2):
        ...
        for dim in range(P2.shape[0]):
            ...
            self.probs[dim]=cv2.resize(P2[dim,:,:],(image_size[1],image_size[0]))
