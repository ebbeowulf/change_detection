    ...
    for cluster_idx1 in scoring.keys():
        ...
        for merge_candidate in scoring[cluster_idx1].keys():
            ...
            if  scoring[cluster_idx1][merge_candidate]>0 and scoring[merge_candidate][cluster_idx1]>0:
                ...
                exportCL=object_pcloud(np.vstack((exportCL.pts,clusters[merge_candidate].pts)),num_samples=FARTHESTP_SAMPLE_SIZE,sample=False)
...
def draw_boxes_around_cluster(fList_new:rgbd_file_list, 
    ...
    for key in rel_imgs:
        ...
        if box_count[whichBox]>0: #don't print images with zero detected points
            tgt_box=np.array(boxes[whichBox][1])
...
def build_change_clusters(fList_new:rgbd_file_list, 
                                fList_renders:rgbd_file_list, 
                                params:camera_params,
                                pcloud_fileName,
