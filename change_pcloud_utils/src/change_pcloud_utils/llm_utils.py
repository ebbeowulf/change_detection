from change_pcloud_utils.llm_query_tools import single_level_results_template, send_data_to_llm, get_model_name
import numpy as np
import cv2
import pdb
...
class before_and_after_cluster_filter():
    def __init__(self, description:list=None, image_scale:float=1.0):
        # self.RESULTS_TEMPLATE=single_level_results_template(["is_new_object","object_type"],
        #                                                     [bool, str],
        #                                                     ["<True/False>","<type of object>"])
        self.RESULTS_TEMPLATE=single_level_results_template(["is_new_object"],
                                                            [bool],
                                                            ["<True/False>"])
    ...
    def merge_images_with_strip(self, img1: np.ndarray, img2: np.ndarray, strip_width: int = 20) -> np.ndarray:
        ...
        merged = np.concatenate((img1[:,:,[2,1,0]], white_strip, img2[:,:,[2,1,0]]), axis=1)
    ...
    def evaluate_multiple_image_pairs(self, 
        ...
        for key in key_list:
            if after_key in all_images[key] and before_key in all_images[key]:
                ...
                # res['image_name']=key
                self.all_results.append(res)
                if 'is_new_object' in res:
                    if res['is_new_object']:
                        cnt_changed+=1
                    cnt_total+=1    
            if cnt_total>=max_pairs:
                break
        if cnt_total==0:
            return 0.5
